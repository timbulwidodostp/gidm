...
qui {
		...
		****grad for twopart 
		 ...
		 *assign normal 
		 ...
		 local end=K
		 forval k=1/`end' {
		 replace `gt`i''=-1*normalden(`xb`i'')/(1-`paiall')    if $ML_y1 ==levs[`k',1] 
		 ...
		 *replace `gt`i''=`gt`i''*(`p`num''-(`w'==`j'))    if $ML_y1 ==A[`j',1] 
		 replace `gt`i''=`v`j''*((`w'==`j')-`p`num'')*normalden(`xb`i'')      if $ML_y1 ==A[`j',1] 
		 }
