...
quietly {
		...
		gen double `dpaidxb`j'' = `pai`j''*(1-`pai`j'')*-1
		...
		*gen qs dqdxb dqdcon
		...
		forval i=1/`end' {
		 tempvar q`i' dqdxb`i'
		 gen double `q`i''=invlogit(`con`i''-`xb')
		 gen double `dqdxb`i''=`q`i''*(1-`q`i'')*-1
		}	
		...
		*assign non-inflate
		...
		if `i'==1 {
			replace `clval'= (1-`paiall')*(invlogit(`con1'-`xb')) if $ML_y1 ==levs[1,1] 
			gen double `p`i''=invlogit(`con1'-`xb') 
			...
			replace `dpdcon`i'`k''=`q`i''*(1-`q`i'') if `k'==`i'
			}
			
		 }
			
		if `i' >1 & `i' <K {
			replace `clval'=(1-`paiall')*(invlogit(`con`i''-`xb')-invlogit(`con`i_''-`xb')) if $ML_y1 ==levs[`i',1]
			gen double `p`i''=(invlogit(`con`i''-`xb')-invlogit(`con`i_''-`xb'))
			...
			tempvar dpdcon`i'`k'
			gen double `dpdcon`i'`k''=0
			replace `dpdcon`i'`k''=-(`q`i_''*(1-`q`i_'')) if (`i'-`k')==1
			replace `dpdcon`i'`k''=`q`i''*(1-`q`i'')      if `k'==`i'
			}
		...
		if `i'==K {
			replace `clval'=(1-`paiall')*(1-invlogit(`con`i_''-`xb')) if $ML_y1 ==levs[K,1]
			gen double `p`i''=(1-invlogit(`con`i_''-`xb')) 
